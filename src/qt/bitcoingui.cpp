...
void BitcoinGUI::createActions()
{
    ...
    signMessageAction->setToolTip(tr("Sign a message to prove you own a FooCoin address"));
    tabGroup->addAction(signMessageAction);

    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setToolTip(tr("Verify a message to ensure it was signed with a specified FooCoin address"));
...
void BitcoinGUI::setMining(bool mining, int hashrate)
{
    if (mining)
    {
        ...
        labelMiningIcon->setToolTip(tr("Mining FooCoins at %1 hashes per second").arg(hashrate));
    }
    else
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/mining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Not mining FooCoins"));
    }
}
